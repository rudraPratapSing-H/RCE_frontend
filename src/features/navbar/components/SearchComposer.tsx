...
export const SearchComposer: React.FC = () => {
  ...
  const { results, matchType, isLoading, error } = useProblemSearch(query);
    ...
    <div className="relative w-full max-w-md">
      ...
      {isOpen && hasContent && (
        <div className="absolute left-0 right-0 top-full z-50 mt-2 rounded-2xl border border-zinc-800 bg-zinc-950 p-2 shadow-2xl">
          {isLoading && <p className="px-3 py-2 text-sm text-zinc-500">Searching problems...</p>}

          {!isLoading && error && <p className="px-3 py-2 text-sm text-red-400">{error}</p>}
